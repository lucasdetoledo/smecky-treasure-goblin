...
const ClassUnderTest = require(`${AppRoot}/app/controllers/artObjectController`)
...
describe('art object controller tests', function () {
  describe('get', function () {
    ...
    beforeEach(function () {
      ...
      find_all_stub = Sinon.stub(ArtObject, 'findAll').resolves(find_all_list)
    })
    afterEach(function () {
    ...
    it('should return a 500 if the find all promise rejects', function (done) {
      // adjust
      find_all_stub.restore()
      find_all_stub = Sinon.stub(ArtObject, 'findAll').rejects(new Error('foo error'))
      // act
      ClassUnderTest.get(req, {
        status: (status_code) => {
          // assert
